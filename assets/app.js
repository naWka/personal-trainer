...
const STATUS = {
  draft: 'черновик', proposed: 'предложено', chosen: 'выбрано',
 ...
 * Пропущенные не показываем вообще. Черновики тоже: план в статусе draft
 * записан, но ещё обсуждается в чате — на сайт он попадёт, когда станет
 * proposed или chosen. Сделанные уводим из выбора: вечером
...
function pickPlan(today) {
  const usable = PLANS.map((p, i) => ({ p, i }))
    .filter(({ p }) => p.status !== 'skipped' && p.status !== 'draft');
